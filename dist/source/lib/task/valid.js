...
function loopRules(taskRules, parentIndex) {
    ...
    taskRules.every((taskRule, index) => {
        ...
        if (!err && taskRule.type === task_1.Type.TYPE_EVENT) {
...
function dataTypeCheckAndRebuild(taskRule) {
    ...
    switch (taskRule.subType) {
        case task_1.TypeDataSubType.SUB_TYPE_MYSQL:
            ...
            break;
        case task_1.TypeDataSubType.SUB_TYPE_REDIS:
...
function domTypeCheckAndRebuild(taskRule) {
    switch (taskRule.subType) {
        case task_1.TypeDomSubType.SUB_TYPE_CLICK:
        case task_1.TypeDomSubType.SUB_TYPE_GET_HTML:
        case task_1.TypeDomSubType.SUB_TYPE_GET_TEXT:
            if (!(taskRule.argments && taskRule.argments.length === 1 && typeof taskRule.argments[0] === 'string')) {
                return new Error('SUB_TYPE_CLICK/SUB_TYPE_GET_HTML/SUB_TYPE_GET_TEXT must have selector & selector must be string');
            }
            break;
        case task_1.TypeDomSubType.SUB_TYPE_KEYBOARD:
        ...
        case task_1.TypeDomSubType.SUB_TYPE_SET_ATTR:
            ...
            break;
        case task_1.TypeDomSubType.SUB_TYPE_SET_INPUT_FILES:
...
function pageTypeCheckAndRebuild(taskRule) {
    switch (taskRule.subType) {
        case task_1.TypePageSubType.SUB_TYPE_SET_COOKIE:
        case task_1.TypePageSubType.SUB_TYPE_DELETE_COOKIE:
...
function timeTypeCheckAndRebuild(taskRule) {
    switch (taskRule.subType) {
        case task_1.TypeTimeSubType.SUB_TYPE_SET_SLEEP:
